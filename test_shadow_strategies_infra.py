...
def test_regime_atr_and_history() -> None:
    ...
    prev = None
    r1 = re.compute_pair_regime("EURUSD", closes, 0.5, 0.4, prev, atr=0.012)
    assert "atr_at_regime_entry" in r1
    assert len(re.get_score_history("EURUSD")) >= 1
    ...
    for _ in range(re.MIN_DWELL_DAYS + 1):
        prev = re.compute_pair_regime(
            "EURUSD", list(reversed(closes)), -2.0, 2.0, prev, atr=0.020,
        )
    if prev.get("changed_this_scan"):
        assert prev.get("atr_at_regime_entry") == 0.020
    assert re.get_atr_at_regime_entry("EURUSD") is not None
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with mock.patch.object(ss, "SHADOW_TRADES_FILE", path):
            ...
            with mock.patch(
                ...
                assert row["strategy_id"] == "PDUMMY01_VALIDATION"
